...
# next step, knowing the bootMode, use that to subset bagged pval calculations to only those using the mode. after that try replotting the graph and make a cutoff
...
cloneVec = sort(unique(unlist(strsplit(unique(unlist(bootMode)), "_"))))
for (cloneName in cloneVec) {
    temp1 = which(bootMode[stability > 20] == cloneName)
    temp2 = seq(1, nrow(rnaScale))[stability > 20][temp1]
